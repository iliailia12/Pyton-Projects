...
# პირველ რიგში, გლობალური ცვლადი (ბიუჯეტი) უნდა იყოს განსაზღვრული
user_budget = float(input("Enter your budget: $"))

# პროდუქციის მონაცემები (ეს თქვენ უკვე გაქვთ, ყველა სექცია და პროდუქტები)
all_sections = {
    "automotive_accessories_sect": {
        "car_seat_cover": {"name": "Car Seat Cover", "price": 35.00, "stock": 20},
        "steering_wheel_cover": {"name": "Steering Wheel Cover", "price": 15.00, "stock": 30},
...
def view_section(user_input):
    user_input = user_input.lower()  # კლებისას გამართავს ტექსტის ფორმატს
...
def process_order(order_input):
    ...
    try:
        order_input = order_input.lower()  # რეგისტრის დამუშავება
