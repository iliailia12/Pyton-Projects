...
all_sections = {
    ...
    "portable_car_pump": {"name": "Portable Car Pump", "price": 30.00, "stock": 20}
     ...
     "garden_items_sect":{   "car_seat_cover": {"name": "Car Seat Cover", "price": 35.00, "stock": 20},
    "steering_wheel_cover": {"name": "Steering Wheel Cover", "price": 15.00, "stock": 30},
    "car_floor_mats": {"name": "Car Floor Mats", "price": 25.00, "stock": 25},
    "car_phone_holder": {"name": "Car Phone Holder", "price": 10.00, "stock": 40},
    "car_vacuum_cleaner": {"name": "Car Vacuum Cleaner", "price": 50.00, "stock": 15},
...
while True:
    ...
    # Section
        ...
        for product_key, product in section.items():  # items() სჭირდება, რადგან ეს ცალკეული პროდუქტია
    ...
    # Whole section
        ...
        if section_name in all_sections:
            ...
            for product_key, product in section.items():
                print(f"{product['name']} - ${product['price']} (Stock: {product['stock']})")
        else:
    ...
    # Order
        ...
        try:
            ...
            if section_name in all_sections:
                section = all_sections[section_name]
                # ვეძებთ პროდუქტს
                product = next((p for p in section.values() if p['name'].lower() == product_name.lower()), None)
                if product:
