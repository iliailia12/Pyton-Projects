// Alphabet Position letter
// 8  TERNARY OPERATOR
// 7  IF ELSE
// 6  SWICH CASE 
// 5  FOR LOOP
// 4  RETURN 
// 3  to work on the website
// 2  Object
// 1  Object IF ELSE



// PHYTON WERSION      Alphabet Position letter shortest path

// def position(letter):
//     if letter == 'a':
//         return 'Position of alphabet: 1'
//     elif letter == 'b':
//         return 'Position of alphabet: 2'
//     elif letter == 'c':
//         return 'Position of alphabet: 3'
//     elif letter == 'd':
//         return 'Position of alphabet: 4'
//     elif letter == 'e':
//         return 'Position of alphabet: 5'
//     elif letter == 'f':
//         return 'Position of alphabet: 6'
//     elif letter == 'g':
//         return 'Position of alphabet: 7'
//     elif letter == 'h':
//         return 'Position of alphabet: 8'
//     elif letter == 'i':
//         return 'Position of alphabet: 9'
//     elif letter == 'j':
//         return 'Position of alphabet: 10'
//     elif letter == 'k':
//         return 'Position of alphabet: 11'
//     elif letter == 'l':
//         return 'Position of alphabet: 12'
//     elif letter == 'm':
//         return 'Position of alphabet: 13'
//     elif letter == 'n':
//         return 'Position of alphabet: 14'
//     elif letter == 'o':
//         return 'Position of alphabet: 15'
//     elif letter == 'p':
//         return 'Position of alphabet: 16'
//     elif letter == 'q':
//         return 'Position of alphabet: 17'
//     elif letter == 'r':
//         return 'Position of alphabet: 18'
//     elif letter == 's':
//         return 'Position of alphabet: 19'
//     elif letter == 't':
//         return 'Position of alphabet: 20'
//     elif letter == 'u':
//         return 'Position of alphabet: 21'
//     elif letter == 'v':
//         return 'Position of alphabet: 22'
//     elif letter == 'w':
//         return 'Position of alphabet: 23'
//     elif letter == 'x':
//         return 'Position of alphabet: 24'
//     elif letter == 'y':
//         return 'Position of alphabet: 25'
//     elif letter == 'z':
//         return 'Position of alphabet: 26'
//     else:
//         return 'Invalid letter'


// letter = input("Enter a letter: ").lower()
// print(position(letter))







// საუკეთესო სინტაქსი 

function position(letter){
    if (letter === 'a'){
        console.log('Position of alphabet: 1')
    } else if (letter === 'b'){
        console.log('Position of alphabet: 2')
    } else if (letter === 'c'){
        console.log('Position of alphabet: 3')
    } else if (letter === 'd'){
        console.log('Position of alphabet: 4')
    } else if (letter === 'e'){
        console.log('Position of alphabet: 5')
    } else if (letter === 'f'){
        console.log('Position of alphabet: 6')
    } else if (letter === 'g'){
        console.log('Position of alphabet: 7')
    } else if (letter === 'h'){
        console.log('Position of alphabet: 8')
    } else if (letter === 'i'){
        console.log('Position of alphabet: 9')
    } else if (letter === 'j'){
        console.log('Position of alphabet: 10')
    } else if (letter === 'k'){
        console.log('Position of alphabet: 11')
    } else if (letter === 'l'){
        console.log('Position of alphabet: 12')
    } else if (letter === 'm'){
        console.log('Position of alphabet: 13')
    } else if (letter === 'n'){
        console.log('Position of alphabet: 14')
    } else if (letter === 'o'){
        console.log('Position of alphabet: 15')
    } else if (letter === 'p'){
        console.log('Position of alphabet: 16')
    } else if (letter === 'q'){
        console.log('Position of alphabet: 17')
    } else if (letter === 'r'){
        console.log('Position of alphabet: 18')
    } else if (letter === 's'){
        console.log('Position of alphabet: 19')
    } else if (letter === 't'){
        console.log('Position of alphabet: 20')
    } else if (letter === 'u'){
        console.log('Position of alphabet: 21')
    } else if (letter === 'v'){
        console.log('Position of alphabet: 22')
    } else if (letter === 'w'){
        console.log('Position of alphabet: 23')
    } else if (letter === 'x'){
        console.log('Position of alphabet: 24')
    } else if (letter === 'y'){
        console.log('Position of alphabet: 25')
    } else if (letter === 'z'){
        console.log('Position of alphabet: 26')
    }
}


// სუფთა სინტაქსი

function position(letter){
    if (letter === 'a'){console.log('Position of alphabet: 1')} else if (letter === 'b'){console.log('Position of alphabet: 2')} else if (letter === 'c'){console.log('Position of alphabet: 3')} else if (letter === 'd'){console.log('Position of alphabet: 4')} else if (letter === 'e'){console.log('Position of alphabet: 5')} else if (letter === 'f'){console.log('Position of alphabet: 6')} else if (letter === 'g'){console.log('Position of alphabet: 7')} else if (letter === 'h'){console.log('Position of alphabet: 8')} else if (letter === 'i'){console.log('Position of alphabet: 9')} else if (letter === 'j'){console.log('Position of alphabet: 10')} else if (letter === 'k'){console.log('Position of alphabet: 11')} else if (letter === 'l'){console.log('Position of alphabet: 12')} else if (letter === 'm'){console.log('Position of alphabet: 13')} else if (letter === 'n'){console.log('Position of alphabet: 14')} else if (letter === 'o'){console.log('Position of alphabet: 15')} else if (letter === 'p'){console.log('Position of alphabet: 16')} else if (letter === 'q'){console.log('Position of alphabet: 17')} else if (letter === 'r'){console.log('Position of alphabet: 18')} else if (letter === 's'){console.log('Position of alphabet: 19')} else if (letter === 't'){console.log('Position of alphabet: 20')} else if (letter === 'u'){console.log('Position of alphabet: 21')} else if (letter === 'v'){console.log('Position of alphabet: 22')} else if (letter === 'w'){console.log('Position of alphabet: 23')} else if (letter === 'x'){console.log('Position of alphabet: 24')}    else if (letter === 'y'){console.log('Position of alphabet: 25')}
        else if (letter === 'z') console.log('Position of alphabet: 26')
}
