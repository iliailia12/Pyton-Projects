...
# Step 2: მთავარი loop
while True:
    user_input = input("Enter a command or section: ")
    ...
    if user_input == "help":
        ...
        section_name, product_name, quantity = parse_order(user_input)
        if section_name in all_sections:
            section = all_sections[section_name]
            if product_exists(section, product_name):
                if section[product_name]['stock'] >= quantity:
                    total_price = section[product_name]['price'] * quantity
                    if total_price <= user_budget:
                        print("Order successful!")
                        section[product_name]['stock'] -= quantity
