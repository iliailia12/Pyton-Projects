...
# Step 0: შევქმნათ all_sections დიკშინარი
all_sections = {
    "electronics_sect": [
        {"name": "Computer", "price": 1200, "stock": 5},
        {"name": "Phone", "price": 800, "stock": 10},
        {"name": "Tablet", "price": 500, "stock": 8}
    ],
    "clothing_sect": [
        {"name": "T-Shirt", "price": 20, "stock": 30},
        {"name": "Jeans", "price": 40, "stock": 20}
    ],
    "shoes_sect": [
        {"name": "Sneakers", "price": 60, "stock": 15}
    ]
    # შეგიძლია აქ დაამატო სხვა სექციებიც
}

# Step 1: მომხმარებლის ბიუჯეტი
while True:
    try:
        user_budget = float(input("Enter your budget: $"))
        break  # თუ სწორად შეიყვანა, გამოვდივართ loop-დან
    except ValueError:
        print("Please enter a valid number for budget.")

# Step 2: მთავარი loop
while True:
    user_input = input("Enter a command or section: ").lower()
    ...
    # Section
    elif user_input in all_sections:
        section = all_sections[user_input]
        print(f"Products in {user_input} within your budget:")
        for product in section:
            if product['price'] <= user_budget:
                print(f"{product['name']} - ${product['price']} (Stock: {product['stock']})")

    # Whole section
    elif user_input.startswith("whole_sect_of_"):
        section_name = user_input.replace("whole_sect_of_", "")
        if section_name in all_sections:
            section = all_sections[section_name]
            print(f"All products in {section_name}:")
            for product in section:
                print(f"{product['name']} - ${product['price']} (Stock: {product['stock']})")
        else:
    ...
    # Order
        ...
        try:
            # მაგალითად: i want to order electronics_sect_Computer_2
            ...
            if section_name in all_sections:
                section = all_sections[section_name]
                # ვეძებთ პროდუქტს
                product = next((p for p in section if p['name'].lower() == product_name.lower()), None)
                if product:
                    if product['stock'] >= quantity:
                        total_price = product['price'] * quantity
                        if total_price <= user_budget:
                            print(f"Order successful! You bought {quantity} x {product_name}.")
                            product['stock'] -= quantity
                            user_budget -= total_price
                        else:
            ...
            print("Invalid order format. Example: i want to order electronics_sect_Computer_2")
