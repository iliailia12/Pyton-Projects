...
# პროდუქტის მონაცემები (ეს თქვენ უკვე გაქვთ, ყველა სექცია და პროდუქტები)


# მომხმარებლის ანდერძები
...
def process_order(order_input):
    try:
        section_name, product_name, quantity = order_input.split("_")
...
# ლუპი, რათა დაადასტუროს შემოთავაზებები
while True:
    user_input = input("Enter a command or section (or type 'exit' to quit): ").strip().lower()
    
    if user_input == "help":
        print("Type 'i want to order [product_name]_[quantity]' to order products.\nOr type a section like 'automotive_accessories_sect' to view products.")
    elif user_input == "exit":
        break
    elif user_input.startswith("i want to order"):
        process_order(user_input[16:].strip())  # remove "i want to order " part
    else:
        view_section(user_input)
