...
all_sections = {
    "automotive_accessories_sect": {
        ...
        "car_floor_mats": {"name": "Car Floor Mats", "price": 25.00, "stock": 25},
        "car_phone_holder": {"name": "Car Phone Holder", "price": 10.00, "stock": 40},
        # დანარჩენი პროდუქტები...
    },
    "electronics_sect": {
        "laptop": {"name": "Laptop", "price": 1000.00, "stock": 5},
        "headphones": {"name": "Headphones", "price": 50.00, "stock": 30},
        "smartphone": {"name": "Smartphone", "price": 800.00, "stock": 10},
    }
    # სხვა სექციები...
}

# პროდუქციის ჩვენება ბიუჯეტის მიხედვით
def view_section(user_input):
    user_input = user_input.strip().lower()  # კლებისას გამართავს ტექსტის ფორმატს
    if user_input in all_sections:
        section = all_sections[user_input]
        print(f"Displaying items from {user_input} within your budget:")
        products_found = False  # Flag to check if any products are found
        for product in section.values():
            if product['price'] <= user_budget:
                print(f"{product['name']} - ${product['price']} (Stock: {product['stock']})")
                products_found = True  # Set to True if a product matches the budget
        if not products_found:
            print("No products found within your budget.")  # თუ არ არის პროდუქტები
    else:
...
while True:
    ...
    if user_input == "help":
        print(help_message)  # აქ გამოიტანს სრულად `help_message`
